...
down: volumes-clean ## Detener y eliminar los servicios
	...
	@docker ps -aq --filter "label=com.docker.compose.service=acestream" | xargs -r docker rm -f 2>/dev/null || true
	@docker ps -aq --filter "name=acestream-" | xargs -r docker rm -f 2>/dev/null || true
